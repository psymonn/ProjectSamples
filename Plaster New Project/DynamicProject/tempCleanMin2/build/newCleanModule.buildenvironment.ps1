...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'newCleanModule'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://www.github/Psymon/newCleanModule'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Psymon. All rights reserved."
        ModuleLicenseURI = 'https://www.github/Psymon/newCleanModule/LICENSE.md'
        ModuleTags = 'tagClean' -split ','
        ModuleAuthor = 'Psymon'
        ModuleDescription = 'new clean module description'
        ...
        # You should NOT enter this key here but rather manually enter it in the newCleanModule.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "newCleanModule.buildenvironment.json"
