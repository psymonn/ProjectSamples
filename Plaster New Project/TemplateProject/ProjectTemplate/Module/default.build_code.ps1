...
$script:OutPutFolder = "$PSScriptRoot\artifacts"
<%
    $folders = @()
    ...
    if ($PLASTER_PARAM_FunctionFolders -contains 'Plugins')
    {
        $folders += "'Plugins'"
    }
...
%>
$script:PsmPath = Join-Path -Path $PSScriptRoot -ChildPath "artifacts\$($script:ModuleName)\$($script:ModuleName).psm1"
$script:PsdPath = Join-Path -Path $PSScriptRoot -ChildPath "artifacts\$($script:ModuleName)\$($script:ModuleName).psd1"
<%
    if ($PLASTER_PARAM_PlatyPS -eq "Yes")
    {
        '$script:HelpPath = Join-Path -Path $PSScriptRoot -ChildPath "artifacts\$($script:ModuleName)\en-US"'
    }
%>
...
<%
    ...
    {
        @'
task ExportHelp -if (Test-Path -Path "$script:ModuleRoot\Docs") {
    New-ExternalHelp -Path "$script:ModuleRoot\Docs" -OutputPath $script:HelpPath
}
'@
    }
%>
